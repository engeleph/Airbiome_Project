...
parser.add_argument('--processors', dest='proc', default=4,
...
try:
    ...
    cwd = "/cluster/project/grlab/projects/metasub/data/Kraken2-database"
    os.chdir(cwd)
    #full_lineage = pd.read_csv('rankedlineage.dmp', sep='|', header=None, index_col=0)
    full_lineage = pd.read_csv('rankedlineage_new.csv',keep_default_na=False, sep=',',header=None, index_col=0)
    full_lineage.drop(columns=full_lineage.columns[-1],  axis=1,  inplace=True)
...
#print(full_lineage)
wd = os.getcwd()
#assembly_summaries['taxid'].astype('int')
test=assembly_summaries.index.duplicated()
#print(np.count_nonzero(test))
assembly_summaries=assembly_summaries.loc[test==False,:]
